...
class RoomService {
public:
	static void JoinRoom(std::shared_ptr<Session> session, blindspot::JoinRoomRequest& pkt) {
		if (session->GetRoom()) {
			// Already in a room
	...
	static void MakeRoom(std::shared_ptr<Session> session, blindspot::MakeRoomRequest& pkt) {
		if (session->GetRoom()) {
			// Already in a room
		...
		int32_t newRoomId = RoomManager::Instance().CreateRoom(title, maxPlayers, password, session->GetPlayerId());
